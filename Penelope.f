...
C                                                                      C
C  - Eliminado el chequeo de rangos de energía en la subrutina START.  C
C                                                                      C
C  - Extendido el tamaño de las variables de nombre de archivo PMFILE  C
C    de 20 a 100 caracteres.                                           C
C                                                                      C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
...
C
      WRITE(IWR,2000)
 2000 FORMAT(/1X,34('*'),/1X,'**   PENELOPE  (version 2008)   **',
     1  /1X,34('*'))
C
...
C
      ...
      DO M=1,NMAT
        IF(EABS(1,M).GE.EMAX) THEN
          ...
          WRITE(IWR,2100)
 2100     FORMAT(/1X,'WARNING: EABS(KPAR,M) has been modified.')
        ENDIF
...
C
 ...
 2001 FORMAT(/1X,'EMIN =',1P,E11.4,' eV,  EMAX =',E11.4,' eV')
      IF(EMAX.LT.EMIN+10.0D0)
     1   CALL PISTOP('The energy interval is too narrow.')
 ...
 2002   FORMAT(/1X,'*** PENELOPE cannot handle ',I2,' different mater',
     ...
     2  'MAXMAT = ',I2,' to MAXMAT = ',I2)
        CALL PISTOP('PEINIT. Too many materials.')
      ENDIF
...
C
 ...
 2103   FORMAT(/1X,'Material data file: ',A)
     ...
     1      PMFILE(M)
          CALL PISTOP('The material data file could not be opened')
        ENDIF
C
...
C
 ...
 2100   FORMAT(/3X,'EMIN =',1P,E11.4,' eV,  EMAX =',E11.4,' eV')
        CALL PISTOP('EGRID. The energy interval is too narrow.')
      ENDIF
C
...
C  ****  E/P inelastic collisions.
     ...
     1  WRI(MAXMAT,NO),QRIE(MAXMAT,NO),QRIP(MAXMAT,NO),KZ(MAXMAT,NO),
     2  KS(MAXMAT,NO),NOSC(MAXMAT)
...
C
      IF(M.GT.MAXMAT) CALL PISTOP('PEMATR. Too many materials.')
C
      LNAME=' PENELOPE (v. 2008)  Material data file ...............'
      READ(IRD,'(A55)') NAME
      IF(NAME.NE.LNAME) THEN
        ...
        CALL PISTOP('PEMATR. Corrupt material data file.')
      ENDIF
      WRITE(IWR,'(A55)') NAME
C
      READ(IRD,5001) LNAME
 ...
 1003 FORMAT(' Number of elements in the molecule = ',I2)
      IF(NELEM(M).GT.30) CALL PISTOP('PEMATR. Too many elements.')
      ZT(M)=0.0D0
      AT(M)=0.0D0
      DO I=1,NELEM(M)
...
C
 ...
 1006 FORMAT(' Number of oscillators =',I3)
      IF(NOSC(M).GT.NO) CALL PISTOP('PEMATR. Too many oscillators.')
...
C
      IF(ABS(EXPT-EXPOT(M)).GT.1.0D-8*EXPOT(M)) THEN
        ...
        CALL PISTOP('PEMATR. Inconsistent oscillator data.')
      ENDIF
C
C  ****  Compton scattering.
C
 ...
 1106   FORMAT(1X,'Number of shells =',I3)
        IF(NOSCCO(M).GT.NOCO) CALL PISTOP('PEMATR. Too many shells.')
      ENDIF
...
C
 ...
 1008 FORMAT(/1X,'*** Stopping powers for electrons and positrons',
     1  ',  NDATA =',I4)
      IF(NDATA.GT.NEGP) CALL PISTOP('PEMATR. Too many data points (1).')
...
C  ****  Compton scattering and pair production.
 ...
 1013 FORMAT(/1X,'*** Compton and pair-production cross sections,  ',
     1  'NDATA =',I4)
      IF(NDATA.GT.NEGP) CALL PISTOP('PEMATR. Too many data points (2).')
      IF(INFO.GE.2) WRITE(IWR,1114)
 1114 FORMAT(/2X,'Energy',5X,'CS_Comp',5X,'CS_pair',/3X,
     1  '(eV)',6X,'(cm**2)',5X,'(cm**2)',/1X,34('-'))
      DO I=1,NDATA
        READ(IRD,*) EIT(I),F2(I),F3(I)
        IF(INFO.GE.2) WRITE(IWR,'(1P,E10.3,5E12.5)') EIT(I),F2(I),F3(I)
        EITL(I)=LOG(EIT(I))
      ENDDO
C
C  ****  Compton scattering.
...
C
      ...
      ENDDO
C
      LNAME=' PENELOPE (v. 2008)  End of material data file ........'
      READ(IRD,'(A55)') NAME
      IF(NAME.NE.LNAME) THEN
        WRITE(IWR,'(/1X,''I/O error. Corrupt material data file.'')')
...
C  ****  'Standard' energy grid.
      ...
      DIMENSION ES(NEGP1),ESS(NEGP1)
      DIMENSION EIT(NEGP1),XG0(NEGP1),PPE(NEGP1),PPT(NEGP1)
C
      M=1
C
      WRITE(6,*) '      '
 ...
 234        FORMAT(/1X,' Enter atomic number and fraction by weight',
     ...
     1    ''by weight ='',1P,E12.5)') LASYMB(IZZ),IZZ,FBW(I)
          IF(I.GT.1) THEN
            DO K=1,I-1
              IF(IZZ.EQ.IZ(M,K))
     1          CALL PISTOP('This element has been declared twice.')
            ENDDO
          ENDIF
...
C
        ...
        ENDDO
        IF(STFM.LT.1.0D-16)
     1    CALL PISTOP('Fractions by weight are too small.')
        DO I=1,NELEM(M)
...
C
  901 CONTINUE
     ...
     1   CALL PISTOP('The allowed material ID numbers are 1-280.')
C
      ...
      DO K1=1,300
        ...
        IF(NELEM(M).GT.30)
     1    CALL PISTOP('NELEM cannot be larger than 30.')
        IF(NELEM(M).LT.1) CALL PISTOP('NELEM must be positive.')
        DO I=1,NELEM(M)
      ...
      ENDDO
  902 CONTINUE
      CALL PISTOP('Abnormal termination of file ''pdcompos.p08''.')
  903 CONTINUE
      CLOSE(UNIT=3)
...
C
     ...
     1    ''molecule ='',1P,E12.5)') LASYMB(IZZ),IZZ,STF(M,I)
          CALL PISTOP('Wrong atomic number.')
        ENDIF
        WRITE(6,'(1X,'' Element: '',A2,'' (Z='',I2,''), atoms/'',
     1    ''molecule ='',1P,E12.5)') LASYMB(IZZ),IZZ,STF(M,I)
        IF(STF(M,I).LE.0.0D0) CALL PISTOP('STF must be positive.')
        IF(I.GT.1) THEN
          DO K=1,I-1
            IF(IZZ.EQ.IZ(M,K))
     1        CALL PISTOP('Element has been declared twice.')
          ENDDO
        ENDIF
        ZT(M)=ZT(M)+IZZ*STF(M,I)
        AT(M)=AT(M)+ATW(IZZ)*STF(M,I)
     ...
     1  1P,E12.5)') ZT(M)
      IF(RHO(M).LE.0.0D0) CALL PISTOP('The density must be positive.')
      WRITE(6,'(1X,'' Mean excitation energy ='',1P,E12.5,
     1  '' eV'')') EXPOT(M)
      VMOL(M)=AVOG*RHO(M)/AT(M)
  904 CONTINUE
...
C        indicator.
 ...
 2005       FORMAT(I3,I3,1X,A5,I3,E12.5,E9.2)
            IF(IIZ.EQ.IZZ) THEN
              ...
              IF(NS.GT.30) THEN
                WRITE(6,'(/1X,''NS ='',I4)') NS
                CALL PISTOP('Too many shells.')
              ENDIF
              IF(IS.LT.1.OR.IS.GT.30) THEN
                WRITE(6,'(/1X,''IS ='',I4)') IS
                CALL PISTOP('Wrong shell number.')
              ENDIF
              IZZT=IZZT+ABS(IIF)
              EB(IZZ,IS)=EIE
              CP(IZZ,IS)=CCP
  ...
  905     CONTINUE
          NSHT(IZZ)=NS
          IF(IZZ.NE.IZZT) CALL PISTOP('Unbalanced charges (element).')
          CLOSE(3)
        ENDIF
      ENDDO
C
...
C
     ...
     1  CALL PISTOP('Unbalanced charges (compound).')
...
C
 ...
 2007   FORMAT(1X,' Enter the oscillator strength Fcb and ',
  ...
  975 CONTINUE
     ...
     1   FP,EP
      IF(FP.GT.ZT(M)+1.0D-13) CALL PISTOP('FP is too large.')
...
C  ****  Check f-sum rule.
     ...
     1  CALL PISTOP('Inconsistent oscillator strength data.')
...
C
      IF(ABS(TST-ZT(M)).GT.1.0D-8*ZT(M)) THEN
        ...
        CALL PISTOP('Inconsistent oscillator-strength data.')
      ENDIF
      EXPT=EXP(EXPT/ZT(M))
      IF(ABS(EXPT-EXPOT(M)).GT.1.0D-8*EXPOT(M)) THEN
     ...
     1      I,FF(I),UUI(I),WWRI(I),FFJ0(I),KKZ(I),KKS(I)
        ENDDO
        CALL PISTOP('Inconsistent oscillator-strength data (2).')
      ENDIF
C
C  ****  Selection of the lowest ionisation energy for inner shells.
C  Only the K, L and M shells with ionisation energy less than that of
...
C
  910 CONTINUE
      ...
      IF(NOST.GT.IFCB+2) THEN
        ...
        IF(KST(I).LT.10.AND.UIT(I).GT.WISCUT) GO TO 911
...
C
  920 CONTINUE
      ...
      IF(NOSTC.GT.2) THEN
        DO 921 I=1,NOSTC-1
        IF(KSC(I).LT.10.AND.UIC(I).GT.WISCUT) GO TO 921
...
C        arrays.
      IF(NOSTC.LT.NOCO) THEN
        ...
        IF(ABS(CSUMT).GT.1.0D-9) THEN
          ...
          CALL PISTOP('Error in grouping the Compton profiles.')
        ENDIF
      ELSE
        RGROUP=RGROUP**2
        GO TO 920
...
C
      WRITE(7,1000)
 1000 FORMAT(1X,
     1  'PENELOPE (v. 2008)  Material data file ...............')
      WRITE(7,1001) NAME
 1001 FORMAT(' Material: ',A62)
      WRITE(7,1002) RHO(M)
 1002 FORMAT(' Mass density =',1P,E15.8,' g/cm**3')
 ...
 1009 FORMAT(1X,'*** Compton and pair-production cross',
     1  ' sections,  NDATA =',I4)
      DO IE=1,NESS
        ...
        WRITE(7,'(1P,E10.3,5E12.5)') EE,CSC,PPT(IE)
      ENDDO
C
      CALL GPHaW(M,7)
C
      WRITE(7,1099)
 1099 FORMAT(1X,
     1  'PENELOPE (v. 2008)  End of material data file ........')
      CLOSE(7)
      RETURN
      END
C  *********************************************************************
...
C  ****  Distant excitations.
      IF(WI.GT.WCCM.AND.WI.LT.E) THEN
        ...
        ENDIF
        IF(QM.LT.QIO) THEN
          XHDL=LOG(QIO*(QM+TREV)/(QM*(QIO+TREV)))*RWI
          XHDT=XHDT0*RWI
        ELSE
          QM=QIO
          XHDL=0.0D0
...
C  ****  Energy and emission angle of the delta ray.
        IF(KS(M,IOSC).LT.10) THEN
          IF(UIO.GT.ECUTR(M)) THEN
            ES=DE-UIO  ! Inner shells only.
          ELSE
            ES=DE
          ENDIF
        ELSE
...
C
      ...
      IF(TST.LT.TS1) THEN
        ...
        Q=QS/(((QS/QIO)*(1.0D0+QIO*RTREV))**RAND(6.0D0)-(QS*RTREV))
        QTREV=Q*(Q+TREV)
        CPPS=EP*(EP+TREV)
...
C
      DO I=1,NOSC(M)
        ...
        WW=WRI(M,I)
        QQ=QRIE(M,I)
        CALL EINaT1(E,UU,WW,QQ,DELTA,WCC,H0,H1,H2,S0,S1,S2,R0,R1,R2)
        SXH0(I)=F(M,I)*H0
        SXH1(I)=F(M,I)*H1
        SXH2(I)=F(M,I)*H2
        SXS0(I)=F(M,I)*S0
        ...
        SXS2(I)=F(M,I)*S2
        SXT0(I)=F(M,I)*R0
        SXT1(I)=F(M,I)*2.0D0*R1
        SXT2(I)=F(M,I)*6.0D0*(R1-R2)
        XH0=XH0+SXH0(I)
        XH1=XH1+SXH1(I)
        XH2=XH2+SXH2(I)
        XS0=XS0+SXS0(I)
...
C                       SUBROUTINE EINaT1
C  *********************************************************************
      SUBROUTINE EINaT1(E,UI,WI,QI,DELTA,WCC,H0,H1,H2,S0,S1,S2,R0,R1,R2)
C
...
C    UI .... ionization energy (eV).
C    WI .... resonance energy (eV).
C    QI .... cutoff recoil energy of the resonant mode (eV).
C    DELTA ... Fermi's density effect correction.
...
C    T1 .... 1st transport cross section for soft colls. (cm**2).
C    T2 .... 2nd transport cross section for soft colls. (cm**2).
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C  ****  Coefficients transferred from subroutine EINaT.
...
C  ****  Distant longitudinal interactions.
        ...
        ENDIF
        IF(QM.LT.QI) THEN
          SDL1=LOG(QI*(QM+TREV)/(QM*(QI+TREV)))
        ELSE
          SDL1=0.0D0
        ENDIF
...
C
      DO I=1,NOSC(M)
        ...
        WW=WRI(M,I)
        QQ=QRIP(M,I)
        CALL PINaT1(E,UU,WW,QQ,DELTA,WCC,H0,H1,H2,S0,S1,S2,R0,R1,R2)
        SXH0(I)=F(M,I)*H0
        SXH1(I)=F(M,I)*H1
        SXH2(I)=F(M,I)*H2
        SXS0(I)=F(M,I)*S0
...
C                       SUBROUTINE PINaT1
C  *********************************************************************
      SUBROUTINE PINaT1(E,UI,WI,QI,DELTA,WCC,H0,H1,H2,S0,S1,S2,R0,R1,R2)
C
...
C  ****  Fundamental constants and related quantities.
      ...
      PARAMETER (PI=3.1415926535897932D0,PIELR2=PI*ELRAD*ELRAD)
C  ****  Coefficients transferred from subroutine EINaT.
      COMMON/CPIN01/EE,CPS,BHA1,BHA2,BHA3,BHA4,MOM
      EXTERNAL PINaDS
C
      H0=0.0D0
      H1=0.0D0
...
C
C  Output arguments:
C    IZZ .... atomic number of the element where ionisation has ocurred.
...
C
 ...
 2001   FORMAT(/1X,'***  Electron impact ionisation cross sections, ',
     ...
     1    CALL PISTOP('ESIaR. Corrupt material data file.')
        ...
        IF(NSHR.GT.9) CALL PISTOP('ESIaR. Too many shells.')
        DO IE=1,NDATA
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdesi'//LDIG2//LDIG1//'.p08'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,*) IZZZ,NSHR
        IF(IZZZ.NE.IZZ) CALL PISTOP('ESIaW. Corrupt data file.')
        IF(NSHR.GT.9) CALL PISTOP('ESIaW. Too many shells.')
        READ(3,'(A12)') LDUMMY
        DO IE=1,NES
...
C
 ...
 2001   FORMAT(/1X,'***  Positron impact ionisation cross sections, ',
     ...
     1    CALL PISTOP('PSIaR. Corrupt material data file.')
        ...
        IF(NSHR.GT.9) CALL PISTOP('PSIaR. Too many shells.')
        DO IE=1,NDATA
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdpsi'//LDIG2//LDIG1//'.p08'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,*) IZZZ,NSHR
        IF(IZZZ.NE.IZZ) CALL PISTOP('PSIaW. Corrupt data file.')
        IF(NSHR.GT.9) CALL PISTOP('PSIaW. Too many shells.')
        READ(3,'(A12)') LDUMMY
        DO IE=1,NES
...
C
      IF(MOM.LT.-1) CALL PISTOP('RLMOM. Error code 0.')
      IF(NP.LT.2) CALL PISTOP('RLMOM. Error code 1.')
      IF(X(1).LT.0.0D0) CALL PISTOP('RLMOM. Error code 2.')
      DO I=2,NP
        IF(X(I).LT.0.0D0) CALL PISTOP('RLMOM. Error code 3.')
        IF(X(I).LT.X(I-1)) CALL PISTOP('RLMOM. Error code 4.')
      ENDDO
C
      RLMOM=0.0D0
      IF(XC.LT.X(1)) RETURN
...
C  ****  Rayleigh scattering.
     ...
     1    IED(NEGP),IEU(NEGP),NE
      ...
      COMMON/CGRA03/QRA(NP,MAXMAT),PRA(NP,MAXMAT),ARA(NP,MAXMAT),
     1  BRA(NP,MAXMAT),PMAX(NEGP,MAXMAT),ITLRA(NP,MAXMAT),
     2  ITURA(NP,MAXMAT)
C
      EXTERNAL RAND
C
...
C  ****  Random sampling (RITA).
      PARAMETER (NM=512)
      COMMON/CRITA/QTI(NM),PACI(NM),AI(NM),BI(NM),NPI,
...
C
      ...
      IF(NPI.NE.NP) THEN
        WRITE(26,*) 'RITA initialisation error in GRAaR.'
        WRITE(26,*) 'The number of fixed grid points is ',NPI
        WRITE(26,*) 'The required number of grid points was ',NP
        CALL PISTOP('RITA initialisation error in GRAaR.')
      ENDIF
      IF(ERRM.GT.1.0D-5) THEN
        WRITE(26,*) 'RITA interpolation error is too large in GRAaR.'
        WRITE(26,*) 'The interpolation error is ',ERRM
        CALL PISTOP('RITA interpolation error is too large in GRAaR.')
      ENDIF
C
...
C
     ...
     2              /((1.0D0-BI(I)*ETAP*ETAP)*CI*(QTI(I+1)-QTI(I)))
            ENDDO
            CALL SIMPSU(DQ,FUN,SUM,NIP)
...
C
      ...
      IF(E.LT.5.0D6) THEN
        DO IO=1,NOSCCO(M)
          IOSC=IO
          SXCO(IO)=FCO(M,IO)*PIELR2*SUMGA(GCOaD,-1.0D0,1.0D0,1.0D-5)
          CS=CS+SXCO(IO)
        ENDDO
      ELSE
...
C
 ...
 2001   FORMAT(/1X,'***  Photoelectric cross sections,  IZ =',I3,
     ...
     1    CALL PISTOP('GPHaR. Corrupt material data file.')
        ...
        IF(NSHR.GT.9) CALL PISTOP ('GPHaR. Too many shells.')
        DO IE=1,NDATA
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdgph'//LDIG2//LDIG1//'.p08'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,*) IZZZ,NSHR
        IF(IZZZ.NE.IZZ) CALL PISTOP('GPHaW. Corrupt file.')
        IF(NSHR.GT.9) CALL PISTOP('GPHaW. Too many shells.')
        NPTAB=0
        DO IE=1,1000
...
C                       SUBROUTINE GPPa
C  *********************************************************************
      SUBROUTINE GPPa(EE,CDTE,EP,CDTP)
C
...
C                       SUBROUTINE GPPaW
C  *********************************************************************
      SUBROUTINE GPPaW(EIT,XG0,NPTAB,M)
C
...
C
C  Data are read from file 'pdrelax.p08', which contains data pertaining
...
C
      OPEN(3,FILE='./pdfiles/pdrelax.p08')
...
C
   ...
   10   FORMAT(5X,'Spline interpolation cannot be performed with',
     1    I4,' points. Stop.')
        CALL PISTOP('SPLINE. N is less than 4.')
      ENDIF
      N1=N-1
      N2=N-2
C  ****  Auxiliary arrays H(=A) and DELTA(=D).
   ...
   11     FORMAT(5X,'Spline X values not in increasing order. Stop.')
          CALL PISTOP('SPLINE. X values not in increasing order.')
        ENDIF
        A(I)=X(I+1)-X(I)
        D(I)=(Y(I+1)-Y(I))/A(I)
      ENDDO
...
C  ****  Check integral limits.
   ...
   10   FORMAT(5X,'Integral limits out of range. Stop.')
        CALL PISTOP('SINTEG. Integral limits out of range.')
      ENDIF
...
C
      ...
      CONS=H*F1O12
      HCONS=0.5D0*CONS
      IF(N.LT.4) CALL PISTOP('Not enough data points.')
      SUMF(1)=0.0D0
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NR=128)
      COMMON/CRNDG3/X(NR),A(NR),B(NR),F(NR),IA(NR),NP
      EXTERNAL RAND
C  ****  Selection of the interval (Walker's aliasing).
      RN=RAND(1.0D0)*(NP-1)+1.0D0
      K=INT(RN)
      TST=RN-K
      IF(TST.LT.F(K)) THEN
        I=K
...
C  Initialisation of the RNDG3 sampling function.
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NM=512)
      COMMON/CRITA/XA(NM),PAC(NM),AA(NM),BA(NM),NPA,
...
C  ****  Interval end points.
      ...
      IF(NPA.NE.N) CALL PISTOP('RNDG30: initialisation error (1).')
      IF(ERRM.GT.1.0D-6)
...
C  ****  Renormalisation.
      ...
      DO I=1,N
        IF(W(I).LT.0.0D0)
     1    CALL PISTOP('IRND0. Negative point probability.')
        WS=WS+W(I)
      ENDDO
      WS=DBLE(N)/WS
      DO I=1,N
...
C  ****  Cutoff and alias values.
      ...
      END
C  *********************************************************************
C                       FUNCTION RITAI (unused, general sampling tool)
C  *********************************************************************
      FUNCTION RITAI()
C
C     Random sampling (by the inverse transform method) of a continuous
C  variable using the RITA (Rational Inverse Transform with Aliasing)
C  method, with binary search within pre-calculated index intervals.
C
C     The needed numerical parameters are determined by the initialisa-
C  tion subroutine RITAI0, which must be invoked before using function
C  RITAI; these parameters are stored in common block /CRITA/.
C
...
C  PDF(X) defined in the interval (XLOW,XHIG). N is the number of points
...
C  first NU grid points are uniformly spaced in (XLOW,XHIG).
C
...
C
      PARAMETER (NIP=51)
      DIMENSION AREA(NM),ERR(NM),C(NM),PDFI(NIP),PDFIH(NIP),SUMI(NIP)
      EXTERNAL PDF
C
      IF(N.LE.16) THEN
        WRITE(26,'('' Error in RITAI0: N must be larger than 16.'',
     1    /,'' N='',I11)') N
        CALL PISTOP('RITAI0: N must be larger than 16.')
      ENDIF
      IF(N.GT.NM) THEN
        WRITE(26,'('' Error in RITAI0: N must be less than NM=512.'',
     1    /,'' N='',I11)') N
        CALL PISTOP('RITAI0: N must be less than NM=512.')
      ENDIF
      IF(XLOW.GT.XHIG-EPS) THEN
        WRITE(26,'('' Error in RITAI0: XLOW must be larger than XHIG.'',
     1    /,'' XLOW='',1P,E13.6,'', XHIG ='',E13.6)') XLOW,XHIG
        CALL PISTOP('RITAI0: XMIN must be larger than XMAX.')
      ENDIF
C
...
C                       SUBROUTINE RITA0M
C  *********************************************************************
      SUBROUTINE RITA0M(XD,XU,XM0,XM1,XM2)
C
C     Calculation of (restricted) momenta of a pdf, PDF(X), obtained
C  from its RITA rational approximation.
C
...
C
      PARAMETER (NM=512)
      COMMON/CRITA/XT(NM),PAC(NM),A(NM),B(NM),NP,
     1             ITTL(NM),ITTU(NM),NPM1
      PARAMETER (NIP=51)
...
C  ****  Value of the RITA rational pdf at the point XI(K).
            ...
            ELSE
              ETA=TAU/CON2
            ENDIF
            FUN(K)=(PAC(I+1)-PAC(I))*(1.0D0+(A(I)+B(I)*ETA)*ETA)**2
     1        /((1.0D0-B(I)*ETA*ETA)*CI*(XT(I+1)-XT(I)))
          ENDDO
C  ****  Simpson's integration.
...
C  ****  Compton electron.
      IF(ES.GT.EABS(1,M)) THEN
        IF(DF.LT.-0.5D0) DF=TWOPI*RAND(3.0D0)
        DFS=DF+PI
        CALL DIRECT(CDTS,DFS,US,VS,WS)
        ILBA(1)=ILB(1)+1
        ILBA(2)=KPAR
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-2) THEN
          ...
          CALL PISTOP('Electron cross section data are corrupt.')
        ENDIF
      ENDDO
C
C  ****  Positrons.
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-2) THEN
          ...
          CALL PISTOP('Positron cross section data are corrupt.')
        ENDIF
      ENDDO
      RETURN
      END
 ...
 2001 FORMAT(/1X,'***  Electron elastic differential cross sections')
     ...
     1    IELEC,ETS(IE),ECS(IE),ETCS1(IE),ETCS2(IE)
        IF(IELEC.NE.-1.OR.ABS(ETSIE-ETS(IE)).GT.0.1D0) THEN
          ...
          CALL PISTOP('Error reading electron elastic DCS data.')
        ENDIF
 ...
 2002 FORMAT(/1X,'***  Positron elastic differential cross sections')
     ...
     1    IELEC,ETS(IE),PCS(IE),PTCS1(IE),PTCS2(IE)
        IF(IELEC.NE.+1.OR.ABS(ETSIE-ETS(IE)).GT.0.1D0) THEN
          ...
          CALL PISTOP('Error reading positron elastic DCS data.')
        ENDIF
...
C
      ...
      DO KE=1,NEGP
        ...
        DO I=1,NP
          ...
          ITUE(I,KE,M)=ITTUI(I)
        ENDDO
        CALL RITA0M(0.0D0,1.0D0,XM0A,XM1,XM2)
        ECS0=CSI
        ECS1=CSI*XM1/XM0A
        ECS2=CSI*XM2/XM0A
        XE0(KE)=ECS0
...
C
        CALL RITA0M(0.0D0,RMUC,XM0,XM1,XM2)
        ECS1=CSI*XM1/XM0A
        ECS2=CSI*XM2/XM0A
        TCS1=2.0D0*ECS1
        TCS2=6.0D0*(ECS1-ECS2)
...
C
     ...
     1      CALL PISTOP('Corrupt data file.')
...
C
      IF(E.LT.49.999D0.OR.E.GT.1.0001D8) THEN
        ...
        CALL PISTOP('Energy out of range.')
      ENDIF
C
      EL=LOG(E)
...
C
      IF(NP.LT.2) CALL PISTOP('RMOMX. Error code 1.')
      IF(X(1).LT.0.0D0.OR.PDF(1).LT.0.0D0) THEN
        WRITE(26,*) 'X(1),PDF(1) =',X(1),PDF(1)
        CALL PISTOP('RMOMX. Error code 2.')
      ENDIF
      DO I=2,NP
        IF(X(I).LT.0.0D0.OR.PDF(I).LT.0.0D0) THEN
          WRITE(26,*) 'I,X(I),PDF(I) =',I,X(I),PDF(I)
          CALL PISTOP('RMOMX. Error code 3.')
        ENDIF
        IF(X(I).LT.X(I-1)) CALL PISTOP('RMOMX. Error code 4.')
      ENDDO
C
      XLOW=MAX(X(1),XD)
